...
void EmotiBit::bmm150GetRegs(uint8_t address, uint8_t *dest, uint16_t len)
{
	for (uint16_t i = 0; i < len; i++)
	{
		...
		while (EmotiBit::getBit(p, BMI160_STATUS_MAG_MAN_OP) != 0)
		{ // wait for MAN_OP to switch back to 0
...
uint8_t EmotiBit::setup(String firmwareVariant)
{
	...
	Serial.println(EmotiBitVersionController::EMOTIBIT_I2C_DAT_PIN);
	Serial.print("I2C clk pin: ");
	Serial.println(EmotiBitVersionController::EMOTIBIT_I2C_CLK_PIN);
	Serial.print("hibernate pin: ");
	Serial.println(EmotiBitVersionController::HIBERNATE_PIN);
	Serial.print("chip sel pin: ");
	...
	if (testingMode == TestingMode::ACUTE || testingMode == TestingMode::CHRONIC)
	{
		...
		Serial.print("_batteryReadPin = ");
		Serial.println(_batteryReadPin);
		Serial.print("Hibernate Pin = ");
		Serial.println(EmotiBitVersionController::HIBERNATE_PIN);
		Serial.print("_vcc = ");
		Serial.println(_vcc);
		...
		Serial.print("LED Driver Current Level = ");
		Serial.println(_emotiBitSystemConstants[(int)SystemConstants::LED_DRIVER_CURRENT]);
	}
	...
	if (!adcCorrectionValues.valid)
	{
		...
		if (adcCorrection.atwincAdcDataCorruptionTest != AdcCorrection::Status::FAILURE && adcCorrection.atwincAdcMetaDataCorruptionTest != AdcCorrection::Status::FAILURE)
		{
			...
			Serial.print(adcCorrectionValues._gainCorrection);
			Serial.print("\toffset correction:");
			Serial.print(adcCorrectionValues._offsetCorrection);
			Serial.print("\tisr offset Corr: ");
			Serial.println(adcCorrectionValues._isrOffsetCorr, 2);
	...
	else
	{
		...
		Serial.print(adcCorrectionValues._gainCorrection);
		Serial.print("\toffset correction:");
		Serial.println(adcCorrectionValues._offsetCorrection);
	}
#endif
...
#ifdef ARDUINO_FEATHER_ESP32
	Serial.print("The main loop is executing on core: ");
...
void EmotiBit::parseIncomingControlPackets(String &controlPackets, uint16_t &packetNumber)
{
	...
	while (_emotiBitWiFi.readControl(packet) > 0)
	{
		...
		if (dataStartChar > 0 || dataStartChar == EmotiBitPacket::NO_PACKET_DATA)
		{
			...
			else if (header.typeTag.equals(EmotiBitPacket::TypeTag::RECORD_END))
			{ // Recording end
				if (_dataFile)
				{
					_dataFile.close();
...
int8_t EmotiBit::updateThermopileData()
{
...
#endif
	if (DIGITAL_WRITE_DEBUG)
		digitalWrite(DEBUG_OUT_PIN_1, HIGH);
	// Thermopile
	int8_t status = 0;
	uint32_t timestamp;
	...
	if (thermopileMode == MODE_STEP)
	{
		...
		else
		{
			...
			{
				if (DIGITAL_WRITE_DEBUG)
					digitalWrite(DEBUG_OUT_PIN_2, HIGH);
				timestamp = millis();
...
float EmotiBit::convertMagnetoY(int16_t mag_data_y, uint16_t data_rhall)
{
	...
	/* Overflow condition check */
	if ((mag_data_y != BMM150_XYAXES_FLIP_OVERFLOW_ADCVAL) && (data_rhall != 0) && (bmm150TrimData.dig_xyz1 != 0))
	{
		/*Processing compensation equations*/
...
float EmotiBit::convertMagnetoZ(int16_t mag_data_z, uint16_t data_rhall)
{
	...
	if ((mag_data_z != BMM150_ZAXIS_HALL_OVERFLOW_ADCVAL) &&
		(bmm150TrimData.dig_z2 != 0) && (bmm150TrimData.dig_z1 != 0) && (bmm150TrimData.dig_xyz1 != 0) && (data_rhall != 0))
	{
		/* Processing compensation equations */
...
void EmotiBit::readSensors()
{
...
#endif // DEBUG
	if (DIGITAL_WRITE_DEBUG)
		digitalWrite(DEBUG_OUT_PIN_0, HIGH);
	...
	if (acquireData.debug)
		pushData(EmotiBit::DataType::DEBUG, micros() - readSensorsBegin); // Add readSensors processing duration to debugBuffer
...
bool EmotiBit::loadConfigFile(const String &filename)
{
	...
	for (size_t i = 0; i < configSize; i++)
	{
		...
		if (_emotiBitWiFi.addCredential(ssid, userid, username, pass) < 0)
		{
			...
			Serial.print("Ignoring credentials beginning: ");
			Serial.println(ssid);
			break;
		}
		else
...
void EmotiBit::processDebugInputs(String &debugPackets, uint16_t &packetNumber)
{
	if (Serial.available())
	{
		...
		else if (c == 'T')
		{
			...
			debugPackets += EmotiBitPacket::createPacket(EmotiBitPacket::TypeTag::EMOTIBIT_DEBUG, packetNumber++, payload, dataCount);
			if (_serialData != DataType::length)
				_serialData = DataType::THERMOPILE;
		}
		else if (c == 'e')
		{
		...
		else if (c == ')')
		{
			...
			debugPackets += EmotiBitPacket::createPacket(EmotiBitPacket::TypeTag::EMOTIBIT_DEBUG, packetNumber++, payload, dataCount);
			if (_serialData != DataType::length)
				_serialData = DataType::TEMPERATURE_0;
		}
		else if (c == '1')
		{
		...
		else if (c == '!')
		{
			...
			debugPackets += EmotiBitPacket::createPacket(EmotiBitPacket::TypeTag::EMOTIBIT_DEBUG, packetNumber++, payload, dataCount);
			if (_serialData != DataType::length)
				_serialData = DataType::TEMPERATURE_1;
		}
		else if (c == 'i')
		{
		...
		else if (c == 'I')
		{
			...
			debugPackets += EmotiBitPacket::createPacket(EmotiBitPacket::TypeTag::EMOTIBIT_DEBUG, packetNumber++, payload, dataCount);
			if (_serialData != DataType::length)
				_serialData = DataType::ACCELEROMETER_X;
		}
		else if (c == 'p')
		{
		...
		else if (c == 'B')
		{
			...
			debugPackets += EmotiBitPacket::createPacket(EmotiBitPacket::TypeTag::EMOTIBIT_DEBUG, packetNumber++, payload, dataCount);
			if (_serialData != DataType::length)
				_serialData = DataType::BATTERY_PERCENT;
		}
		else if (c == '-')
		{
			chipBegun.NCP5623 = false;
...
{
	Serial.print("The data acquisition is executing on core: ");
	Serial.println(xPortGetCoreID());
...
void EmotiBit::bufferOverflowTest(unsigned int maxTestDuration, unsigned int delayInterval, bool humanReadable)
{
	...
	Serial.print("~Time @first overflow: ");
	Serial.println(timeFirstOverflow);
}
